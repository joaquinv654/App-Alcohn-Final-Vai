import { 
  ...
  ChevronLeft,
  ChevronRight,
  LogOut // 1. Importar el ícono de LogOut
} from 'lucide-react';
...
import { useAuth } from '@/lib/auth/AuthContext'; // 2. Importar nuestro hook de autenticación
import { supabase } from '@/lib/supabaseClient'; // 3. Importar el cliente de Supabase
...
export function Sidebar() {
  ...
  // 4. Usar el hook de autenticación
  const { user } = useAuth(); 
  ...
  const handleLogout = async () => {
    ...
    // No necesitamos redirigir, el 'onAuthStateChange' en AuthContext
    // y la lógica en App.tsx lo harán automáticamente.
  };
  ...
  return (
    ...
    >
      ...
      {/* User Profile */}
      <div className="border-t border-border/50 flex p-4 justify-between items-center">
        <div className="flex items-center gap-3 justify-start min-w-0">
          <div className="rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-lg transition-all duration-300 hover:scale-110 hover:shadow-xl h-10 w-10 flex-shrink-0">
          ...
          <div className={cn(
            ...
            {/* 6. Mostrar el email real del usuario */}
        ...
        {/* Botón de Cerrar Sesión (visible solo si la barra está expandida) */}
        <div className={cn(
          "transition-all duration-700 ease-in-out overflow-hidden",
